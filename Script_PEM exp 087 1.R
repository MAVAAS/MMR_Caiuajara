...
data<-read.table("MMRdata1.txt", header=T)
tree<-read.nexus("untitled2.trees.nex")
#tree<-drop.tip(tree, setdiff(tree$tip.label, data$species))
plot(tree)
data<-ReorderData(tree, data, taxa.names = 1)
data[,c(2:3)]<-log1p(data[,c(2:3)])
grloc <- getGraphLocations(tree,data[is.na(data[,"MMR087"]),"species"])
sporder <- match(attr(grloc$x,"vlabel")[grloc$x$vertex$species],data[,"species"])


# Extract eigenvectors from the tree

PEMfs <- PEM.fitSimple(y=data[sporder,"MMR087"],x=NULL,w=grloc$x,d="distance",sp="species",lower=0,upper=1)
...
PEMfs <- list()
  for(m in aux) {
    PEMfs[[m]] <- PEM.fitSimple(y=data[sporder,"MMR087"],
                                        x=data[sporder,m],w=grloc$x,d="distance",sp="species",lower=0,upper=1)
  PEMfs[["none"]] <- PEM.fitSimple(y=data[sporder,"MMR087"],
...
PEMAIC <- list()
for(m in aux) {
  PEMAIC[[m]] <- lmforwardsequentialAICc(y=data[sporder,"MMR087"],
                                            x=data[sporder,m,drop=FALSE],object=PEMfs[[m]])
  PEMAIC[["none"]] <- lmforwardsequentialAICc(y=data[sporder,"MMR087"],object=PEMfs[["none"]])
}
...
# Predicting missing values for MMR087

m <- "Qdot"

atr <- data[is.na(data[,"MMR087"]),m,drop=FALSE]
resultsPEM<-predict(object=PEMfs[[m]],targets=grloc,lmobject=PEMAIC[[m]],newdata=atr,interval="confidence")

# Predicted MMR087, with upper and lower limits of the confidence interval for the prediction:
...
data<-read.table("MMRdata1.txt", header=T) # Reload data to plot it with log10 conversion
...
par(mar=c(4,1,1,1))
plot(tree2,cex=0.50)
lab <- log10(range(data[,"MMR087"],na.rm=TRUE)) ; lab <- c(floor(lab[1L]),ceiling(lab[2L]))
plot(NA,ylim=c(1L,length(tree2$tip.label)),xlim=lab,axes=FALSE,xlab="MMR (mLO2 h-1 g-0.87)")
lab <- lab[1L]:lab[2L]
axis(1,at=lab,label=10^lab)

# Plot the observed, fitted, and predicted values
points(y=match(data[!is.na(data[,"MMR087"]),"species"],tree2$tip.label[ordered_tips]),
       x=log10(data[!is.na(data[,"MMR087"]),"MMR087"]),pch=22,bg="white",cex=2)
points(y=match(data[!is.na(data[,"MMR087"]),"species"],tree2$tip.label[ordered_tips]),
       x=log10(exp(yfit)),pch=4,cex=2)
points(y=match(data[is.na(data[,"MMR087"]),"species"],tree2$tip.label[ordered_tips]),
       x=log10(exp(ypred$value)),pch=22,bg="black",cex=2)
arrows(y0=match(data[is.na(data[,"MMR087"]),"species"],tree2$tip.label[ordered_tips]),
       y1=match(data[is.na(data[,"MMR087"]),"species"],tree2$tip.label[ordered_tips]),
...
dataCV<-subset(data, !is.na(data$MMR087))
dataCV2<-subset(data, !is.na(data$MMR087))
m <- "Qdot"
...
for (i in 1:nrow(dataCV)) {
  ...
  grloc <- getGraphLocations(treeCV,dataCV[is.na(dataCV[,"MMR087"]),"species"])
  sporder <- match(attr(grloc$x,"vlabel")[grloc$x$vertex$species],dataCV[,"species"])
  PEMfs<-PEM.fitSimple(y=dataCV[sporder,"MMR087"],
                       x=dataCV[sporder,m],w=grloc$x,d="distance",sp="species",lower=0,upper=1)
  PEMAIC<-lmforwardsequentialAICc(y=dataCV[sporder,"MMR087"],x=dataCV[sporder,m,drop=FALSE],object=PEMfs)
  atr <- dataCV[is.na(dataCV[,"MMR087"]),m,drop=FALSE]
...
predata<-vector(length=length(na.omit(data$MMR087)))
...
predata <- sapply(seq_along(na.omit(data$MMR087)), function(i) {
...
wilcox.test(dataCV2$MMR087,dataCV2$predata, paired=T)
